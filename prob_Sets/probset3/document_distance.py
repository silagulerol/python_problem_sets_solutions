...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    def lamb(e):
        return abs(count(e, freq_dict1) - count(e, freq_dict2))
    
    def alpha(e):
        return count(e, freq_dict1) + count(e, freq_dict2)
    ...
    def similarity(u):
        ...
        return 1 -(total_lamb/total_alpha)
            
    return round(similarity(universe),2)
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    max = 0
    for i in universe:
        if max < (freq_dict1[i] + freq_dict2[i]):
            max = freq_dict1[i] + freq_dict2[i]
        
    max_list = []
    for i in universe:
        if (freq_dict1[i] + freq_dict2[i])== max:
            max_list.append(i)
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    d_freq = get_frequencies(my_list)
    ##???????????????????  NEDEN BURADA TF = d_freq DİYEMİYORUM  ????????????????? 
    TF = {}
    for i in my_list:
        TF[i] = d_freq[i]/len(my_list) 
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for idx,i in enumerate(file_paths):
        inFile = open(file_paths[idx],'r')
        line = inFile.read().strip()
        for char in string.punctuation:
             line = line.replace(char, "")
        inFile.close()
        lists.append(line.lower().split())
    ...
    IDF = {}
    for w in universe:
        IDF[w] = math.log10(len(file_paths)/ d_freq[w])
